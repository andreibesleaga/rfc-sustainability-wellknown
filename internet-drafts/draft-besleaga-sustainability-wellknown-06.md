...
A publisher that signs its document MUST publish the signature at the path `/.well-known/sustainability-data.jws` on the same origin, retrievable under the same conditions as the document itself (HTTPS, `GET` and `HEAD`, no authentication). The resource is registered as a companion of the primary suffix in the IANA Considerations section, and carries a signature over the primary resource and nothing else.
...
* **Moved Internationalization Considerations after IANA Considerations**, per the section order of RFC 7322, Section 4, and added a note on the display of `provider` strings written in or mixing right-to-left scripts (Unicode Bidirectional Algorithm; isolation from surrounding text).
