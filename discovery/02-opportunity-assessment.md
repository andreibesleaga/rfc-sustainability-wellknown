...
| Spec not yet an RFC | Revision -04 is the latest posted revision, under ISE review (it carries the simplified omission-based data model introduced in -03, schema label "2.0", and the `sustainability-data` URI rename). Well-known registration only needs Specification Required + Expert Review (RFC 8615). |
