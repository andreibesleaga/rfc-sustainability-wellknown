...
The JSON object MAY contain the following OPTIONAL keys to align with the {{GHG-PROTOCOL}}, the European Sustainability Reporting Standards climate standard ESRS E1 {{ESRS-E1}}, and other sustainability recommendations:
...
Range constraints (the non-negativity rules and the 0-100 bound on `renewable-energy`) and the unit defaults are prose rules of this document; they are not captured by the schemas above, and validating implementations enforce them at the application layer.
