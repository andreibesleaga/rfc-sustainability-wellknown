...
const USAGE = "Usage: sustainability-fetch <origin> [--target=] [--period=] [--granularity=] [--format=json|csv|ndjson] [--strict] [--etag=] [--allow-http]\n" +
    "\n" +
...
async function runCli(argv) {
    ...
    if (!allowInsecure && new URL(origin).protocol === "http:") {
        console.error(`Refusing plain HTTP for ${origin}: the draft requires HTTPS — pass --allow-http for a local/CI origin.`);
        return 2;
    }
    if (opts.strict) {
        const report = await (0, conformance_1.runConformanceChecks)(origin, undefined, { allowInsecure });
        for (const c of report.checks) {
            ...
            const label = c.outcome === "pass" ? "PASS" : c.outcome === "warn" ? "WARN" : c.level === "MUST" ? "FAIL" : "WARN";
